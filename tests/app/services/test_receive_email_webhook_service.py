...
class TestReceiveEmailWebhookService:
    """Unit tests for ReceiveEmailWebhookService."""
    ...
    @pytest.fixture
    def service(self, mock_db: AsyncMock) -> ReceiveEmailWebhookService:
        """ReceiveEmailWebhookService instance."""
        return ReceiveEmailWebhookService(mock_db)

    def test_validate_webhook_payload_email_provider_format(
        self, service: ReceiveEmailWebhookService
    ) -> None:
        """Test validation of email provider format webhook payload."""
        webhook_data = {
            "from_email": "sender@example.com",
            "to_email": "recipient@example.com",
            "subject": "Test Subject",
            "content": "Test email content",
            "x_message_id": "message-123",
        ...
        assert result.body == "Subject: Test Subject\n\nTest email content"
        assert result.provider_message_id == "message-123"
        assert result.provider_type == "email"
        assert result.attachments == []
        assert isinstance(result.timestamp, datetime)

    def test_validate_webhook_payload_email_provider_with_html(
        self, service: ReceiveEmailWebhookService
    ) -> None:
        """Test validation of email provider format with HTML content."""
        webhook_data = {
            "from_email": "sender@example.com",
            "to_email": "recipient@example.com",
            "subject": "Test Subject",
            "content": "Plain text content",
            "html_content": "<html><body><h1>HTML Content</h1></body></html>",
            "x_message_id": "message-123",
            "timestamp": "2024-11-01T14:00:00Z",
        }
    ...
    def test_validate_webhook_payload_unified_format(
        ...
        assert result.provider_message_id == "message-123"
        assert result.provider_type == "email"  # type: ignore
        assert result.attachments == ["https://example.com/attachment.pdf"]
        assert isinstance(result.timestamp, datetime)
    ...
    def test_validate_webhook_payload_invalid_email_format(
        self, service: ReceiveEmailWebhookService
    ) -> None:
    ...
    async def test_find_or_create_conversation_existing(
        self, service: ReceiveEmailWebhookService, mock_db: AsyncMock
    ) -> None:
    ...
    async def test_process_webhook_full_flow(
        ...
        with (
            ...
            mock_find_conv.assert_called_once_with(
            ...
            assert webhook_request.from_address == "sender@example.com"
            assert webhook_request.to_address == "recipient@example.com"
            assert webhook_request.body == "Test email content"
            assert webhook_request.provider_message_id == "message-123"
            assert webhook_request.provider_type == "email"
